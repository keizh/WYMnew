...
import Asyncfetchtvshow from "../store/actions/TVAction";
...
function TVDetailsComp() {
  const { info } = useSelector((state) => state.TVDetailsReducer);
  ...
  return info ? (
    <div
      className=" relative h-fit pb-[200px] w-full bg-red-100 text-black py-8 px-4 sm:px-10 md:px-12 md:px-16 xl:px-24 overflow-x-hidden overflow-y-auto flex flex-col gap-10"
      style={{
    ...
    >
      ...
      {(info.detail.seasons !== null) & (info.detail.seasons.length > 0) ? (
        <>
          <div className="flex justify-center">
            <h1 className="text-3xl text-white font-bold">Seasons</h1>
          </div>
          <HorizontalCardsComp
            trendingCategoryData={info.detail.seasons}
            media_type="tv"
            type="seasons"
          />
        </>
